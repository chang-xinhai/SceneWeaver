...
1.front_against: child_obj's front faces to parent_obj, and stand very close, such as chair and dining table.
2.front_to_front: child_obj's front faces to parent_obj's front, and stand very close, such as chair and desk, coffee table and sofa.
...
6.ontop: child_obj is placed on the top of parent_obj. Such as monitor and desk, vase and table.
7.on: child_obj is placed on the top of or inside parent_obj. Such as book and shelf.
...
For against_wall, it may include objects placed on the ground (sofa) as well as hanging on the wall (picture).
You should first look through each object to check if it should be against wall. You can take regular usage habits into account. If the object is always placed against wall (toilet, shelf) or on the wall (shower head), you **MUST** add against_wall relation regardless of the layout to make the scene reasonable.
But if the object is not always against wall, you must check the distance.
You can also use side_against_wall instead of against_wall if the object's side is close to the wall, such as a bed or a bathhub with side against the wall.
